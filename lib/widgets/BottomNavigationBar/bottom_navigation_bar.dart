...
class _CustomBottomNavigationBarState extends State<CustomBottomNavigationBar>
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          child: AnimatedContainer(
              ...
              child: Opacity(
                opacity: isEnable ? _yController?.value??0 : 1,
                child: Icon(icon,
                    color: isEnable
  ...
  Widget _buildBackground() {
    ...
    return CustomPaint(
      painter: BackgroundCurvePainter(
          _xController?.value??0 * MediaQuery.of(context).size.width,
          Tween<double>(
            begin: Curves.easeInExpo.transform(_yController?.value??0),
            end: inCurve.transform(_yController?.value??0),
          ).transform(_yController?.velocity.sign??0 * 0.5 + 0.5),
  ...
  void _handlePressed(int index) {
    if (_selectedIndex == index || (_xController?.isAnimating??false)) return;
